...
def cbar_zero_centered(start, end, num_bins=10, cmap="RdBu", rescale=False):
    """
    ...
    Args:
        ...
        num_bins (int, optional): number of colors in the colorbar. Defaults to 10.
    ...
    stop_point = 0.5 / zero_fraction  # e.g. the "1" endpoint is moved to 0.5/0.4 = 1.25
    ...
    if rescale:
        ...
        # this is really sneaky trick I found
        # og_cmap(x) only wants inputs between 0-1, but..
        # if you pass in values greater than 1, it just repeats the max color
        # e.g. og_cmap(1.1) == og_cmap(1.5)
        new_colors = og_cmap(np.linspace(0, stop_point, og_cmap.N))

    shifted_cmap = mcolors.ListedColormap(new_colors)

    # rely on mpl's clever auto-generation of tickmarks
